...
def _validate_imports(target_file: str, file_content: str) -> list[str]:
    ...
    # Skip type files - they reference types being created elsewhere in the same run
    ...
    for i, line in enumerate(lines, 1):
        ...
        # Skip node_modules (no leading dot)
        if not module_path.startswith("."):
            continue

        # Resolve relative path from target file's directory
        base_dir = Path(target_file).parent
        resolved = (base_dir / module_path).resolve()

        # Check if file exists with common TS extensions
        # Check if file exists with common TS extensions
        found = any(
...
class AIManager:
    ...
    def _handle_code_task(self, state: Dict[str, Any], task: Dict[str, Any]) -> bool:
        ...
        for attempt in range(1, MAX_RETRIES + 2):  # initial + retries
            ...
            # Check if reviewer gave a complete file despite saying FAIL
            if "```" in review and not review.strip().upper().startswith("PASS"):
                # Extract code block from reviewer response
                import re
                code_match = re.search(r'```(?:typescript|tsx)?\n(.*?)\n```', review, re.DOTALL)
                if code_match:
                    extracted = code_match.group(1).strip()
                    if len(extracted) > 50:  # It's a real file, not a snippet
                        save_text(Path(target_file), extracted)
                        task["status"] = "done"
                        task["last_error"] = ""
    ...
    def _review_file(self, task: Dict[str, Any], proposed_content: str, target_file: str) -> str:
        ...
        review_prompt += (
                    ...
                    "Check: correct TypeScript syntax, proper type annotations, logic matches task instructions, "
                    "exports are correct, no 'any' types unless necessary, arrays of objects are never joined directly.\n"
                    "IMPORTANT: Do NOT fail because imported files do not exist yet — they will be created in later tasks."
                )
